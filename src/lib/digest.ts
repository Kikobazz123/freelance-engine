...
export function controlText(batchId: string, sendable: number, blocked: number): string {
  return [
    `BATCH ${batchId}`,
    ``,
...
export async function postDigest(batchId: string): Promise<{
  ...
  if (!drafts.length) {
    await sendPlain(`Batch ${batchId} is empty — no new candidates to write to.`);
    return { messages: 0, sendable: 0, blocked: 0 };
  }
  ...
  const card = await sendPlain(
    ...
    sendable.length ? controlButtons(batchId, sendable.length) : undefined,
  );
