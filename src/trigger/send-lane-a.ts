...
export const sendLaneA = schedules.task({
  ...
  run: async () => {
    ...
    if (room === 0) {
      logger.info("daily cap already reached, not staging", { cap, already });
    ...
    const { batchId, staged, blocked } = await stageBatch(room, (s) => logger.info(s));

    if (!staged && !blocked) {
      logger.info("no new candidates to stage");
